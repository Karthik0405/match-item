...
class App extends Component {
  state = {
    categoryItem: tabsList[0].tabId,
    isGameOn: false,
    score: 0,
    imgIs: imagesList[0].imageUrl,
    timer: 60,
  }
  ...
  gettingFilterdList = () => {
    ...
    const categotyList = imagesList.filter(
      eachItem => eachItem.category === categoryItem,
    )
    return categotyList
  }

  componentDidMount = () => {
    this.timeIs = setInterval(this.tick, 1000)
  }
  ...
  gettingScore = id => {
    const {imgIs} = this.state
    const imageFound = imagesList.find(eachItem => eachItem.id === id)
    if (imageFound.imageUrl === imgIs) {
      const reloadImg =
  ...
  render() {
    const {categoryItem, score, timer, imgIs, isGameOn} = this.state
    const filterList = this.gettingFilterdList()
    return (
      <div>
          ...
          <ul className="navbar-score-timer-container">
            <li>
              <p className="navbar-score">
                score:<span className="navbar-span-score">{score}</span>
              </p>
            </li>
            <li className="navbar-timer-container">
              ...
              />
              <p className="navbar-span-score">{timer} sec</p>
            </li>
          </ul>
        </nav>
        <div className="match-bg-container">
          {!isGameOn && (
            <div>
              <img src={imgIs} alt="match" className="display-img" />
              <ul className="tab-list-items-container">
                {tabsList.map(eachItem => (
                  <TabItems
                    eachItem={eachItem}
                    key={eachItem.tabId}
                    gettingTabId={this.gettingTabId}
                    isActive={eachItem.tabId === categoryItem}
                  />
                ))}
              </ul>
